...
{
    public class GetCrawlJobDataBehaviour : BlackmoresProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(BlackmoresConstants.KeyName.ApiKey, nameof(BlackmoresCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task BlackmoresCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Blackmores.Core.BlackmoresCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
